...
def build_scenario() -> dict:
    ...
    # Запасное изделие того же типа: на нём проверяется замена сгоревшего
    # устройства - у нового свой device_id и свой ключ, а датапоинты те же.
    devices["sim-spare-light-01"] = {
        "name": "Торшер (замена)",
        "kind": "device",
        "dps": {"20": False, "22": 700, "23": 500, "21": "white"},
    }
    ...
    # Батарейный датчик температуры: заряд (dp 4) объявлен в конфигурации, но
    # устройство его НЕ присылает - так ведёт себя реальное железо, которое
    # шлёт температуру часто, а заряд раз в сутки. На нём проверяется, что
    # отчёт по соседнему датапоинту не затирает известный заряд.
    devices["sim-temp-sensor-01"] = {
        "name": "Датчик температуры (батарейный)",
        "kind": "device",
        "dps": {"1": 237, "2": 303},
    }
    return devices
